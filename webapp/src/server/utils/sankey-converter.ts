...
export function convertToSankeyData(transactions: DisplayTransaction[]): SankeyData {
  ...
  const incomeBySubcategory = new Map<string, { category: string; amount: number }>();
  
  for (const transaction of incomeTransactions) {
    // カテゴリ別集計
    const categoryAmount = incomeByCategory.get(transaction.category) || 0;
    incomeByCategory.set(transaction.category, categoryAmount + transaction.absAmount);
  ...
  const expenseBySubcategory = new Map<string, { category: string; amount: number }>();
  
  for (const transaction of expenseTransactions) {
    // カテゴリ別集計
    const categoryAmount = expenseByCategory.get(transaction.category) || 0;
    expenseByCategory.set(transaction.category, categoryAmount + transaction.absAmount);
